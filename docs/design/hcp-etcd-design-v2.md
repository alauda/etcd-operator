...
**怎么升级。** HyperShift 里每个 hosted 控制面组件（含 etcd）都是 management 集群的 workload。升级分两层：组件自身版本滚动（换镜像、滚动重启 Pod）；承载它们的 management 节点走标准节点滚动升级（逐个 drain、换机、重启）。

**与普通节点升级的差异。** 普通节点 drain 无状态约束，Pod 换地方重建即可；而 hosted etcd 是 quorum 敏感的有状态服务，drain 一个 etcd 节点＝中断一个成员，必须受 PDB 严格控制、一次只动一个，等被换成员带原数据重新加入、集群健康后才动下一个。
